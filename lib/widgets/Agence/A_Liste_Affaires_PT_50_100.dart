...
class A_Liste_Affaires_PT_50_100 extends StatefulWidget {
  @override
  A_Liste_Affaires_PT_50_100State createState() =>
      A_Liste_Affaires_PT_50_100State();
}

class A_Liste_Affaires_PT_50_100State
    extends State<A_Liste_Affaires_PT_50_100> {
  ...
  void Reload() async {
    ...
    if (DbTools.SelEtabID >= 0) {
      ...
      print("Reload DbTools.SelEtabID ${DbTools.SelEtabID}");


      await DbTools.getInventairesEtabID50100(DbTools.SelEtabID);
      Filtre();
  ...
  void initLib() async {
    ...
    ListEtablissementNewCts.forEach((element) async {
...
//                    print("onCurrentCellActivated ${SelCol}");
  ...
  Widget DD_Period() {
    ...
    return DropdownDatePicker(
      ...
      {
         SelMonth = int.parse(value!) ;
         print("${SelMonth} ${Selyear}");
      },


      onChangedYear: (value)       {
        Selyear = int.parse(value!) ;
        print("${SelMonth} ${Selyear}");
      },
  ...
  Widget DD_Etabs() {
    print(
        ">>>>>>>> ListEtablissementNewCts.length ${ListEtablissementNewCts.length}");

    print(
        ">>>>>>>> ListEtablissementNewCts[0] ${ListEtablissementNewCts[0].Libelle}");
    ...
    return Row(
      ...
      children: [
        ...
        DropDown<Etablissement>(
          items: ListEtablissementNewCts,
          initialValue: ListEtablissementNewCts[DbTools.SelEtabIDi],
          icon: Icon(
            Icons.expand_more,
            color: gColors.secondary,
          ),
          onChanged: (newValue) async {
            ...
            print("DD_Etabs selectedEtablissement ${DbTools.selectedEtablissement.Libelle} ${DbTools.selectedEtablissement.IsPT}");

            Reload();
          },
          customWidgets:
              ListEtablissementNewCts.map((z) => buildDropDownRowEtab(z))
                  .toList(),
          isExpanded: false,
        ),
      ],
